...
/**
 * Sets the byte order to read byte stream.	
 * @type {int}
 * @values - 00 : Big Endian
 *           01 : Little Endian
 * @private
 */
...
ol.parser.WKB.prototype.readBytesToUInt8_ = function(str) {
  if (this.index_ > str.length - 1) throw new Error(this.index_+'/'+(str.length-1)+' Index out of range.');
...
ol.parser.WKB.prototype.readBytesToUInt32_ = function(str) {
  if (this.index_ > str.length - 4) throw new Error(this.index_+'/'+(str.length-1)+' Index out of range.');
  var data = [];
  for(var i = 0; i < 4; ++i) {
    data[this.byteOrder_ ? 3-i : i] = str[i+this.index_];
  }
  var result = ((data[0] << 24) + (data[1] << 16) + (data[2] << 8) + (data[3] << 0));
  this.index_ += 4;
  return result;
};
...
ol.parser.WKB.prototype.readBytesToDouble_ = function(str) {
  if (this.index_ > str.length - 8) throw new Error(this.index_+'/'+(str.length-1)+' Index out of range.');
  var data = [];
  for(var i = 0; i < 8; ++i) {
  ...
  var mantissa = data[7]+data[6]*Math.pow(2,8*1)+data[5]*Math.pow(2,8*2)+data[4]*Math.pow(2,8*3)
                +data[3]*Math.pow(2,8*4)+data[2]*Math.pow(2,8*5)+(data[1]&15)*Math.pow(2,8*6)+Math.pow(2,52);
